...
if (!isset($_SESSION["nombres"]) || ($_SESSION["tipo"] !== "comprador" && $_SESSION["tipo"] !== "admin")) {
...
<div class="header_1">
    ...
    <div class="separador_1">
        <h2 class="separador_2">Mis compras</h2>
    </div>
    <?php
   ...
   // Enviar consulta
   $instruccion = "select pisos.* from usuario,pisos,comprados where pisos.codigo_piso = comprados.codigo_piso and comprados.usuario_comprador=usuario.usuario_id and usuario.usuario_id='$id';";
   ...
   if ($nfilas > 0) {
      for ($i = 0; $i < $nfilas; $i++) {
         ...
         print("        </div>\n");
         print("        <form method='post' action='vendido.php'>\n");
         print("            <input type='hidden' name='Codigo_piso' value='" . $resultado['Codigo_piso'] . "'>\n");
         print("            <input type='submit' value='VENDER' class='portada_5ro'>\n");
         print("        </form>\n");
         print("    </div>\n");
      }
      print("</div>\n");
