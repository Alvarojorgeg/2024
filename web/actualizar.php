...
// Datos
$nuevonombre = $_REQUEST["user"];
$nuevaclave = password_hash($_REQUEST["password"], PASSWORD_DEFAULT); // Hashear la contraseña
$nuevocorreo = $_REQUEST["email"];
$tipo = $_REQUEST["tipo"];
$id = $_SESSION['id'];

// Consulta
$query = "UPDATE usuario SET clave='$nuevaclave', nombres='$nuevonombre', correo='$nuevocorreo', tipo_usuario='$tipo' WHERE usuario_id='$id';";

if (mysqli_query($conn, $query)) {
    echo "<div class='portada_1'>";
    echo "<div class='cuadrado3'><p class='correcto'>DATOS ACTUALIZADOS</p></div>";
    echo "</div>";
} else {
    echo "<div class='portada_1'>";
    echo "<div class='cuadrado3'><p class='nohayresultado'>DATOS NO ACTUALIZADOS</p></div>";
    echo "</div>";
}
